...
public class ContentServer{
    ...
    public static void main(String[] args){
                ...
                String test = ""; // If we are testing this file we will pass in 3rd argument

                if (args.length > 2) test = args[2]; // Storing the test number in the variable

                if (test.equals("testcase_1")) { // Testing 1
                    ...
                    String xml = xstream.toXML(whole_feed); // Converting the object to xml

                    System.out.println(formatXml(xml)); // Prinitng out the xml to see if the parsing is working
                    
                    feed whole_feed_temp = (feed)xstream.fromXML(xml); // Converting xml back to object

                    whole_feed.print(); // Printing to see if the object is converted back correctly

                } else if (test.equals("testcase_2")) { // Testing 2
                    ...
                    try {
                            ...
                            for (int i = 0; i<3; i++) {
                                ...
                                if (i == 0) xml = "INSERT"; // checking for error code 400
                                else if (i == 1) xml = "PUT"; // Checking for error code 204
                                else if (i == 2) xml = "PUT\n\n<?xml version=\"1.0\""; //Checking for error code 500
                                ...
                                output_to_server.writeUTF(xml + "LamportClock:"+ obj.clock.get_LamportClock()); //Updating the lamport clock and sending the xml to Aggregation server

                                String id_and_lamport = input_from_client.readUTF();
                                String id = id_and_lamport.split("LamportClock:")[0];
                                obj.clock.tick(Integer.parseInt(id_and_lamport.split("LamportClock:")[1])); // Updating the lamport clock and getting the id from the Aggregation server
                                
                                String lamport_and_responseID = input_from_client.readUTF(); // Getting the data from the server
                                String responseID = lamport_and_responseID.split("LamportClock:")[0];
                                obj.clock.tick(Integer.parseInt(lamport_and_responseID.split("LamportClock:")[1])); // Updating the lamport clock and getting the responseID

                                System.out.println(responseID); //Printing out the id to check if we are getting the correct code
